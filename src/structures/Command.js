...
class Command extends Base {
	constructor(client, store, file, options = {}) {
		...
		this.extendedHelp =
			options.extendedHelp ?? "No extended help provided.";
	...
	async verifyMember(msg, member, required) {
		...
		const guildMember = await msg.guild.members
			.fetch(user)
			.catch(() => null);
	...
	async verifyChannel(msg, channel, required = false) {
		...
		const chan = await this.client.channels
			.fetch(match[1])
			.catch(() => null);
	...
	verifyRole(msg, rolename, required = false) {
		...
		const role = msg.guild.roles.cache.find(
			(role) =>
				role.id === rolename || role.name.toLowerCase() === rolename,
		);
	...
	async run(msg) {
		return msg.send(
			...
			} does not provide a \`run()\` implementation.${
