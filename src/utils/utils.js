	...
	static embedContains(embed, str) {
		...
		if (embed.fields && embed.fields.length) {
			for (const field of embed.fields) {
				if (field.name && field.name.toLowerCase().includes(str))
					return true;
				if (field.value && field.value.toLowerCase().includes(str))
					return true;
			}
		}
	...
	static async walk(dir, options = {}, results = new Map(), level = -1) {
		...
		if (
			...
			(typeof options.depthLimit === "undefined" ||
				level < options.depthLimit)
		) {
			await Promise.all(
				(
					await readdir(dir)
				).map((part) =>
					Utils.walk(path.join(dir, part), options, results, ++level),
				),
			);
		}
